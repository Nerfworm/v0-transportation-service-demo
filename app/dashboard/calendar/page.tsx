...
function transformRequests(data: any[]): ReviewRequest[] {
  return (data || []).map((r: any, idx: number) => {
    ...
    if (r.requested_dropoff_time) {
      ...
      arrivalDate = date.toLocaleDateString("en-CA")
  ...
  return (
    <div className="relative">
      <input
        ...
        required
        value={value && typeof value === "object" ? `${value.first_name} ${value.last_name}` : search || value || ""}
      ...
      {showDropdown && (
        <div className="border rounded bg-white max-h-40 overflow-y-auto shadow-md w-full mt-1" style={{ position: "relative" }}>
          {driversList
            .filter(d => (d.first_name + " " + d.last_name).toLowerCase().includes(search.toLowerCase()))
            .map(d => (
              <div
                key={d.supabase_uid}
                ...
                onMouseDown={e => { e.preventDefault(); onChange(d); setSearch(`${d.first_name} ${d.last_name}`); setShowDropdown(false) }}
              >
...
export default function CalendarPage() {
  ...
  // Fetch requests on mount and every 60s
  useEffect(() => {
    const cached = localStorage.getItem("calendar_requests")
    if (cached) {
      try { setRequests(transformRequests(JSON.parse(cached))) } catch {}
    }
    const fetchAndCache = () => {
      fetchGetRequests("")
        .then(res => {
        ...
        .catch(err => setRequestError(err.message || "Failed to fetch requests"))
    }
    fetchAndCache()
  ...
  // Fetch drivers when approve or add transport modal opens
  useEffect(() => {
    if (!showApproveModal && !showAddTransportModal) return
    fetchGetDrivers()
      .then(data => { if (data.valid && Array.isArray(data.data)) setDriversList(data.data) })
      .catch(() => {})
  }, [showApproveModal, showAddTransportModal])

  async function handleApprove() {
    if (!selectedRequest || !selectedDriver || !vehicle || !pickupTime || !dropoffTime) return
    try {
      await fetchConfirmRequest(
        selectedRequest.id, "Approved", undefined, selectedDriver.id, vehicle,
        `${selectedRequest.arrivalDate}T${pickupTime}:00Z`,
        `${selectedRequest.arrivalDate}T${dropoffTime}:00Z`
      )
      ...
      setRequestError(null)
      fetchGetRequests("").then(res => setRequests(transformRequests(res.data)))
    } catch (err: any) {
  ...
  async function handleReject() {
    ...
    try {
      await fetchConfirmRequest(selectedRequest.id, "Denied", rejectReason)
  ...
  const goToPreviousWeek = () => setCurrentDate(prev => { const d = new Date(prev); d.setDate(d.getDate() - 7); return d })
  const goToNextWeek = () => setCurrentDate(prev => { const d = new Date(prev); d.setDate(d.getDate() + 7); return d })
  ...
  return (
    <DashboardLayout>
      <div className="flex flex-row w-full h-full overflow-hidden">
        ...
        <div className="flex-1 max-w-[70vw] min-w-[700px] bg-card rounded-xl shadow-lg overflow-hidden mr-8 h-[1150px] flex flex-col">
          <div className="p-4 border-b border-border flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
            <div className="flex items-center gap-4 flex-wrap">
              ...
              <Button variant="default" className="ml-4" onClick={() => setShowAddTransportModal(true)}>+ Add Transport</Button>
                  ...
                  <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl z-50 w-[420px] max-w-[95vw] p-6">
                    ...
                    >
                      ...
                      <div className="space-y-2">
                        ...
                        <Select value={addTransportForm.houseName} onValueChange={value => setAddTransportForm(f => ({ ...f, houseName: value }))}>
                          <SelectTrigger id="houseName"><SelectValue placeholder="Select House Name" /></SelectTrigger>
                          <SelectContent>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select value={filterDriver} onValueChange={setFilterDriver}>
                <SelectTrigger className="w-36"><SelectValue placeholder="All Drivers" /></SelectTrigger>
                <SelectContent>
            ...
            <div className="min-w-[900px] flex-1 flex flex-col h-full">
              <div className="grid grid-cols-8 border-b border-border">
                ...
                {weekDates.map((date, index) => (
                  <div key={index} className="p-3 text-center border-r border-border last:border-r-0">
                    ...
                    <div className={`text-lg font-bold ${date.toDateString() === new Date().toDateString() ? "bg-primary text-primary-foreground rounded-full w-8 h-8 flex items-center justify-center mx-auto" : ""}`}>
                      {date.getDate()}
                    </div>
                  </div>
                ))}
              ...
              {HOURS.map((hour, hourIndex) => (
                <div key={hour} className="grid grid-cols-8 border-b border-border last:border-b-0">
                  <div className="p-3 text-xs text-muted-foreground border-r border-border flex items-start justify-end">{hour}</div>
                  {weekDates.map((date, dayIndex) => {
                    const approvedRequests = requests.filter(r => {
                      if (r.status !== "approved" && r.status !== "Approved") return false
                      if (r.arrivalDate !== date.toISOString().split("T")[0]) return false
                      return getHourIndex(r.arrivalTime) === hourIndex
                    })
                    return (
                      <div key={dayIndex} className="p-1 min-h-16 border-r border-border last:border-r-0 hover:bg-muted/50 cursor-pointer transition-colors">
          ...
          {requestError ? (
            ...
            <ul className="divide-y divide-border">
              {requests.filter(r => r.status === "Pending").map(r => {
                ...
                if (r.arrivalTime?.includes(":")) {
                  const [h, m] = r.arrivalTime.split(":")
                  if (!isNaN(Number(h)) && !isNaN(Number(m))) {
                    const hour = parseInt(h, 10)
                    formattedTime = `${hour % 12 === 0 ? 12 : hour % 12}:${m} ${hour >= 12 ? "PM" : "AM"}`
                  }
                }
                return (
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl z-50 w-96 max-w-[90vw] p-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                {selectedRequest.transport && (
                  <>
                    {selectedRequest.transport.pickup_time && <div><strong>Pickup Time:</strong> {formatTime12h(new Date(selectedRequest.transport.pickup_time).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false }))}</div>}
                    {selectedRequest.transport.dropoff_time && <div><strong>Dropoff Time:</strong> {formatTime12h(new Date(selectedRequest.transport.dropoff_time).toLocaleTimeString("en-US", { hour: "2-digit", minute: "2-digit", hour12: false }))}</div>}
                    {selectedRequest.transport.vehicle && <div><strong>Vehicle:</strong> {selectedRequest.transport.vehicle}</div>}
                    {selectedRequest.transport.account && <div><strong>Driver:</strong> {selectedRequest.transport.account.first_name} {selectedRequest.transport.account.last_name}</div>}
                  </>
                )}
        ...
        {/* ─── Request Detail + Approve/Reject Modal ───────────────────── */}
        {selectedRequest && !showEventInfoModal && (
          <>
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl z-50 w-96 max-w-[90vw] p-6">
              ...
              <div className="mb-4">
                ...
                <input
                  ...
                  onChange={e => { setDriverSearch(e.target.value); setSelectedDriver(null); setShowDriverDropdown(true) }}
                  onFocus={() => setShowDriverDropdown(true)}
                  onBlur={() => setTimeout(() => setShowDriverDropdown(false), 100)}
                />
                {showDriverDropdown && (
                  <div className="border rounded bg-white max-h-40 overflow-y-auto shadow-md w-full mt-1" style={{ position: "relative" }}>
                    {driversList
                      .filter(d => (d.first_name + " " + d.last_name).toLowerCase().includes(driverSearch.toLowerCase()))
                      .map(d => (
                        <div
                          key={d.supabase_uid}
                          className={`px-3 py-2 cursor-pointer transition-colors ${selectedDriver?.supabase_uid === d.supabase_uid ? "bg-gray-200 text-gray-900 font-semibold" : "bg-white text-gray-900"} hover:bg-gray-100 hover:text-black`}
                          onMouseDown={e => { e.preventDefault(); setSelectedDriver(d); setDriverSearch(`${d.first_name} ${d.last_name}`); setShowDriverDropdown(false) }}
                        >
                          {d.first_name} {d.last_name}
                        </div>
                      ))}
              ...
              <div className="mb-4">
                ...
                <input type="text" className="w-full border rounded p-2" placeholder="Enter vehicle..." value={vehicle} onChange={e => setVehicle(e.target.value)} />
              </div>
              <div className="mb-4">
                <label className="block text-sm font-semibold mb-1">Pickup Time</label>
                <input type="time" className="w-full border rounded p-2" value={pickupTime} onChange={e => setPickupTime(e.target.value)} />
              </div>
              <div className="mb-4">
                <label className="block text-sm font-semibold mb-1">Dropoff Time</label>
                <input type="time" className="w-full border rounded p-2" value={dropoffTime} onChange={e => setDropoffTime(e.target.value)} />
              </div>
              <div className="flex gap-3 mt-4">
        ...
        {showRejectModal && (
          <>
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl z-50 w-96 max-w-[90vw] p-6">
              ...
              <textarea className="w-full border rounded p-2 mb-4 min-h-[80px]" placeholder="Enter reason for rejection..." value={rejectReason} onChange={e => setRejectReason(e.target.value)} />
              <div className="flex gap-3 mt-4">
        ...
        {selectedEvent && (
          <>
            ...
            <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white rounded-lg shadow-2xl z-50 w-96 max-w-[90vw] p-8">
              <button onClick={() => setSelectedEvent(null)} className="absolute top-4 right-4 text-gray-400 hover:text-gray-600 text-2xl">×</button>
              <h2 className="text-2xl font-bold mb-6 text-gray-900">{selectedEvent.title}</h2>
              <div className="space-y-4">
                {selectedEvent.driver && <div><p className="text-sm text-gray-500 font-semibold">DRIVER</p><p className="text-gray-900">{selectedEvent.driver}</p></div>}
                {selectedEvent.description && <div><p className="text-sm text-gray-500 font-semibold">DESCRIPTION</p><p className="text-gray-900">{selectedEvent.description}</p></div>}
                {selectedEvent.location && <div><p className="text-sm text-gray-500 font-semibold">LOCATION</p><p className="text-gray-900">{selectedEvent.location}</p></div>}
                {selectedEvent.passengers && <div><p className="text-sm text-gray-500 font-semibold">PASSENGERS</p><p className="text-gray-900">{selectedEvent.passengers}</p></div>}
                {selectedEvent.notes && <div><p className="text-sm text-gray-500 font-semibold">NOTES</p><p className="text-gray-900">{selectedEvent.notes}</p></div>}
                <div><p className="text-sm text-gray-500 font-semibold">TIME</p><p className="text-gray-900">{HOURS[selectedEvent.hour]}</p></div>
              </div>
              <button onClick={() => setSelectedEvent(null)} className="w-full mt-8 bg-gray-900 text-white py-2 rounded-lg hover:bg-gray-800 transition-colors font-medium">Close</button>
            </div>
          </>
        )}
